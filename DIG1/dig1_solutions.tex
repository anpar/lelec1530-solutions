...
On trouve donc
\begin{align*}
    \tau &= \SI{65}{\pico\second} \\
    \alpha &= \SI{2.05}{\volt} \\
    t^* &= \SI{53.48}{\pico\second}. \\
\end{align*}
...
On trouve $R_{eq} = \SI{7.715}{\kilo\ohm}$.

Dans la zone linéaire (approximation linéaire du régime triode),
on avait $\tau = C_{rout} / (k_n v_{OV})$. Par conséquent,
$R_{lin} = 1 / (k_n v_{OV}) = \SI{3.846}{\kilo\ohm}$.

Finalement,
\[k = \frac{R_{eq}}{R_{lin}} = 2.006 \approx 2.\]

\subsection*{Question 4: Scaling de la technologie CMOS}

\[R_{eq} = k R_{lin} = \frac{k}{k_n (V_{DD} - V_t)} = \SI{7.164}{\kilo\ohm} \]
\[t_{PHL} = \SI{10}{\pico\second} \]
