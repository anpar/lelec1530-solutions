...
Pour la suite, on pose $C_{ox0} = \SI{625}{aF}$ (capacité pour le NMOS d'un inverseur unitaire).

\subparagraph{Calcul de $t_0$}
Dans le pire cas, chaine de 3 NMOS de largeur 3.
Capacité du noeud $\overline{C_{n+1}}$:
\begin{itemize}
    \item Sortie des NMOS: $3/2 C_{ox0}$ (on prend en compte un seul NMOS, l'autre est déjà pris en compte
        par le terme en $N^2$ de la formule du délai)
    \item PMOS (de precharge): $2C_{ox0}$ (car effet Miller)
    \item Entrée de l'inverseur: $9/2 C_{ox0}$
    \item Entrée du PMOS du circuit de la somme: $9 C_{ox0}$ (car effet Miller)
\end{itemize}

On a donc les paramètres suivants:
\begin{itemize}
    \item Capacité totale de sortie: $C_{out0} = 17 C_{ox0}$.
    \item Capacité de grille des NMOS: $3C_{ox0}$
    \item Résistance des NMOS: $R_N/30$
    \item Nombre de NMOS: $N = 3$
\end{itemize}

On a donc
\[t_0 = 0.35 N^2 (3C_{ox0}) R_N/30 + 0.7 C_{out0} N R_N/30 = \SI{32}{ps}\]
...
\begin{itemize}
\item Capacité de sortie de l'inverseur $3C_{ox0}$
\item NMOS: $9/2 C_{ox0}$ (largeur 3, effet Miller)
\item PMOS: $2\cdot 5 \cdot 2 \ cdot 3/2 C_{ox0} = 30 C_{ox0}$ (deux PMOS de largeur 5 avec effet Miller)
\end{itemize}
Capacité totale: $C_{out1} = 70.5 C_{ox0}$.
...
\begin{itemize}
    \item NMOS: $3C_{ox0}/2$ (car effet Miller)
    \item PMOS ($\overline{C_{n+1}}$): $3C_{ox0}$ (un seul PMOS de largeur 3)
    \item Entrée de l'inverseur: $9/2 C_{ox0}$
\end{itemize}

On a donc les paramètres suivants:
\begin{itemize}
    \item Capacité totale de sortie: $C_{out2} = 9 C_{ox0}$.
    \item Capacité de grille des PMOS: $10 C_{ox0}$
    \item Résistance des PMOS: $R_P/100$
    \item Nombre de PMOS: $N = 5$
\end{itemize}
On a donc
\[t_2 = 0.35 N^2 (10C_{ox0}) R_P/100 + 0.7 C_{out2} N R_P/100 = \SI{51}{ps}\]

\subparagraph{Calcul de $t_3$}
Sortie d'un inverseur:
\[t_3 = 0.7 \cdot 3 \cdot C_{ox0} R_N/10 = \SI{4.5}{ps}\]
