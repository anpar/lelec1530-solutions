...
% TODO: définir i_out sur le schéma
\begin{figure}[h]
	\centering
		\begin{tikzpicture}[scale=0.92]
			...
			(7,0) to[short, i=$i_{out}$] (8,0) to[R=$R_{out}$] (8,-2)
			(8,-2) node[ground]{}
...
\begin{figure}[h]
	\centering
		\begin{tikzpicture}
			\draw
			(0, 2) to[cI,l_=$g_m v_{in}$] (0,0)
                        (0,2) to[short, -*] (1.5,2) node[anchor=west] {$v_{s2}$} to[R=$r_0$] (1.5,0) -- (0,0)
			(0.75,0) node[ground]{}
			(0, 2) to[cI=$g_m v_{s2}$] (0,4)
			(1.5,2) to[R,l_=$r_0$] (1.5,4) -- (0,4)
                        (1.5, 4) to[short, -o] (2,4) node[anchor=west] {$v_{out}$}
			;
		\end{tikzpicture}
...
\begin{center}
    \begin{tikzpicture}
        \draw
        (0, 0) to[cV,l^=$A_{v0}v_{in}$] (0, 2) to[R=$R_{out}$] (2, 2) to[R=$R_{out}$] (2, 0) -- (0, 0)
        (1, 0) node[ground]{}
        ;
    \end{tikzpicture}
\end{center}
